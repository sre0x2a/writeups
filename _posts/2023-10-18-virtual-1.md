...
Note since the username and serial storage locations can be thought of as `r4` and `r5` respectively the (disassembly)[https://sre0x2a.github.io/writeups/assets/files/insn.txt] file for simplicity will show all the but the instructions 0x52 and 0x69 by their register "name."
...
The first two bytes of the serial string must be the ASCII character equivalent to the length of the username string in hexadecimal. Leading zeroes are to be applied for lengths under 16 bytes.
...
The ASCII representation of total number of on bits summed from every byte in the username string in hexadecimal are the third and fouth bytes (characters) of the serial string.
