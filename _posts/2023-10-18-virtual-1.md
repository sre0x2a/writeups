...
Back in the function at 0xbe, 0x9a is called with its returned value copied to the `r2` register and then shifted 4 bits to the left. 0x9a is called again with the return value added to the `r2` register result. The function then returns with `r2` holding the converted bytes.
